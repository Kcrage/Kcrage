// Builds the signature "neofetch --whoami" card (dark + light) for the profile README.
...
export async function fetchLiveStats(username) {
  const headers = { "User-Agent": "kcrage-profile-card" };
...
// ---- Right column: 2x3 stat-dashboard tiles, staggered slide + fade ----
...
function buildRightTiles(c, profile, startTime) {
  const tiles = [
    tile(c, { label: "CGPA", value: `🎓 ${profile.stats.cgpa}`, border: c.cyan }),
    tile(c, { label: "PROJECTS SHIPPED", value: `🚀 ${profile.stats.projectsShipped}`, border: c.cyan }),
    tile(c, { label: "DSA FOCUS", value: `🧠 ${profile.stats.dsaFocus}`, border: c.green }),
    tile(c, { label: "STATUS", value: `📟 ${profile.status.state}`, border: c.amber, glow: true, caption: profile.status.note }),
    tile(c, { label: `GRIND \`${profile.grind.namespace}\``, value: `🔥 ${profile.grind.verdict}`, border: c.magenta, caption: profile.grind.caption }),
    tile(c, { label: "STACK SCOPE", value: `🛠️ full-stack`, border: c.cyan, caption: profile.stats.stackScope }),
  ];
...
function buildCard(theme, profile, liveStats) {
  ...
  const title = `${profile.handle}@dev: neofetch --whoami`;
